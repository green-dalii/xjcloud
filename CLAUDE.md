...
- `components/shared/TypewriterText.tsx` — 打字机动效组件
  - Props：`text`、`pauseAtIndex`、`breakAtIndex`、`midPauseMs`、`typeInterval`、`initialDelay`、`onDone`
  - 逐字打字 → 中途暂停 → 继续 → 完成后触发 `onDone` 回调
  - `breakAtIndex` 在指定位置插入 `<br>`（移动端换行）
  - 完成后光标变为金色常驻闪烁
...
### 首页特殊处理
- 首页 (`page.tsx`) 双屏滚动设计：Hero 全屏首屏 + 双面板第二屏
- 首屏 Hero：背景图 Ken Burns 缩放动画 + 深色渐变 overlay + TypewriterText 打字机标题 + 副标题淡入 + 机械键帽风格 CTA 按钮
- 动画链：打字机完成 → 副标题淡入（1.2s）→ CTA 淡入（0.8s）→ 3.5s 后无限抖动循环（0.5s 高频横向震动）
- CTA 按钮 6 层 box-shadow 侧壁堆叠 3D 效果，按压触底 + 释放环形脉冲
- 第二屏双面板左右并排（移动端纵向堆叠），左侧「我要探索」→ `/explore`，右侧「我要造趣」→ `/host`
